...
class PointSampler(object):
    def __init__(self, output_size, decimate):
    ...
    def __call__(self, mesh):
        ...
        for i in range(int(len(sampled_faces) * self.decimate)):
...
def default_transforms(decimate):
    return transforms.Compose([PointSampler(1024, decimate), Normalize(), ToTensor()])
...
class PointCloudData(Dataset):
    def __init__(
        ...
        folders = [
            ...
            if os.path.isdir(Path(root_dir) / dir)
        ]
